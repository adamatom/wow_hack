...
//         }
        ...
        public bool Update()
        {
            lock (this)
            {
                ...
                {
                    y = env.Memory.ReadFloat(ptrBase + 0x798);
                    x = env.Memory.ReadFloat(ptrBase + 0x79C);
                    z = env.Memory.ReadFloat(ptrBase + 0x7A0);
                    facing = env.Memory.ReadFloat(ptrBase + 0x7A8);
                ...
                if (type == WoahObjectType.GameObject)
                {
                    ...
                    x = ReadStorageFloat((int)EDynamicobjectFields.DYNAMICOBJECT_POS_X);
                    y = ReadStorageFloat((int)EDynamicobjectFields.DYNAMICOBJECT_POS_Y);
                    z = ReadStorageFloat((int)EDynamicobjectFields.DYNAMICOBJECT_POS_Z);
                    int basePtr = env.Memory.ReadInteger(ptrBase + 0xD30);
                    if (basePtr != 0)
                    {
                        basePtr = env.Memory.ReadInteger(basePtr + 0x04);
        ...
        public bool BobberBobbing
        {
            get
            {
                ...
                return ((env.Memory.ReadInteger(ptrBase + 0xBC)&0xFF) == 1) ? true : false;//F4 as of 2.4.2? maybe 2.4.1, 0xfC before
            }
        }
        #endregion
...
//         }
        ...
        public bool Unit_IsDead
        {
            get
            {
                if (Type != WoahObjectType.Unit && Type != WoahObjectType.Player)
                    return false;
                if (Unit_Health == 1 || Unit_Health == 0)
                    return true;
                else
                    return false;
            }
...
//         }
        ...
        public WoahObject Unit_ChannelObject
        {
            get
            {
                ...
                ulong guid = (ulong)ReadStorageLong((int)EUnitFields.UNIT_FIELD_CHANNEL_OBJECT*4);
                if (guid != 0x0)
